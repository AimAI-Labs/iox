...
/// 构建合并后的 Webview 初始化脚本（包含通用缩放持久化与可选的 DOM 自动填充脚本）
pub fn build_initialization_script(dom_injection: Option<&str>) -> String {
...
mod tests {
    ...
    fn test_build_initialization_script_combining() {
        let script_none = build_initialization_script(None);
        assert_eq!(script_none, ZOOM_PERSISTENCE_SCRIPT);

        let custom_code = "console.log('hello');";
        let script_with_injection = build_initialization_script(Some(custom_code));
